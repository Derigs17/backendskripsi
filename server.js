...
// endpoint untuk menambahkan kegiatan baru
app.post('/addKegiatan', upload.single('gambar'), async (req, res) => {
  ...
  // Membuat nama file unik dengan timestamp untuk menghindari bentrok
  ...
  try {
    ...
    const { data, error: uploadError } = await supabase
      .storage
      .from('gambarkegiatan')  // Nama bucket Supabase
    ...
    const getPublicUrl = supabase
      .storage
      .from('gambarkegiatan') // Nama bucket Anda di Supabase
      .getPublicUrl(filePath);
    console.log(getPublicUrl);
    // Cek jika terjadi error saat mengambil public URL
    const publicURL = getPublicUrl?.data?.publicUrl; // Mendapatkan URL publik dari data yang diupload
    if (!publicURL) {
      console.error('Error getting public URL');
      return res.status(500).send('Error getting public URL');
    }
    console.log('Public URL:', publicURL); // Log URL publik untuk debugging  
